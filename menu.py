    ...
    def run(self):
        if self.class_joueur is None:
            ...
            while True:
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        ...
                        if bouton_start.collidepoint(pygame.mouse.get_pos()):
                            player = Archer("Sarah", camera_group.carte.get_waypoint('Spawn'), [player_sprite] + list(camera_groups.values()))
                            player_position = {"x": player.pos.x, "y": player.pos.y}
                            self.save_data.save_player_position(player_position)
                            self.save_data.save_player_map(camera_group.carte.map_name)
                            self.save_data.save_player_life(player.get_HP())
                ...
                texte_bienvenue = font_grand.render("Bienvenue dans le meilleur workshop de France", True, (255, 255, 255))
                ...
                self.screen.blit(texte_start, (bouton_start.x + 10, bouton_start.y + 10))
